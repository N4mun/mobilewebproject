...
import { doc, setDoc, serverTimestamp, getDoc } from 'firebase/firestore'; // เพิ่ม getDoc
    ...
    const handleEmailAuth = async () => {
        ...
        try {
            if (isLogin) {
                ...
                Alert.alert('Success', 'สร้างบัญชีสำเร็จ!');
            }
        } catch (error) {
    ...
    const handlePhoneAuth = async () => {
        ...
        try {
            ...
            if (error.code === 'auth/captcha-check-failed') {
                errorMessage = 'การยืนยัน reCAPTCHA ล้มเหลว กรุณาลองใหม่';
            } else if (error.code === 'auth/invalid-phone-number') {
                errorMessage = 'เบอร์โทรศัพท์ไม่ถูกต้อง กรุณาตรวจสอบ';
            }
                    ...
                    {!confirmationResult ? (
                        <>
                            <TextInput
                                ...
                                placeholder="Phone Number (เช่น 0812345678)"
